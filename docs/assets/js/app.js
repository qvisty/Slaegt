...
function statusMaerke (p) {
  ...
  if (p.status === 'spor') return '<span class="maerke spor">Familieoplysning</span>'
...
function miniHtml (n) {
  ...
  if (!p) {
    ...
    return '<div class="mini tom"><div class="prik" style="background:var(--papir-2);color:var(--blaek-3)">?</div><div><div class="rel">' + esc(relation(n)) + '</div><div class="n muted">' + 'P.t. ukendt' + '</div></div></div>'
  }
  const prik = p.portraet && visDetaljer(p) ? '<img src="' + esc(p.portraet) + '" alt="">' : esc(initialer(p))
  const farve = side(n)
  return '<a class="mini" href="#/person/' + n + '"><div class="prik" style="background:var(--' + farve + '-lys);color:var(--' + farve + ')">' + prik + '</div><div><div class="rel">' + (p.status === 'spor' ? 'Familieoplysning · ' : '') + esc(relation(n)) + '</div><div class="n">' + esc(fuldtNavn(p)) + '</div><div class="lille">' + esc(levetid(p)) + '</div></div></a>'
}
...
function boksSvg (n, x, y, w, h, yderst) {
  ...
  const rel = (st === 'spor' ? 'Familie · ' : '') + relation(n)
  const linjer = []
  if (p) {
    ...
    linjer.push('<text class="br" x="' + (x + 10) + '" y="' + (y + 16) + '">' + esc(rel.length > 28 ? 'Anenr. ' + n : rel) + '</text>')
    linjer.push('<text class="ba" x="' + (x + 10) + '" y="' + (y + 38) + '">' + 'P.t. ukendt' + '</text>')
  }
...
function vifteSvg () {
  ...
  for (let g = 1; g < antalGen; g++) {
    ...
    for (let i = 0; i < antal; i++) {
      ...
      const p = P.get(n)
      const titel = '<title>' + esc((p ? fuldtNavn(p) + '. ' : '') + relation(n) + '. Anenummer ' + n + (p ? '' : '. P.t. ukendt')) + '</title>'
      const seg = '<path class="vifte-seg" d="' + d + '" fill="' + fyld + '"' + (st === 'undersoeges' ? ' stroke="var(--vent)"' : st === 'spor' ? ' stroke="var(--blaek-3)"' : '') + '>' + titel + '</path>'
      let tekst = ''
      if (p) {
        const am = (a0 + a1) / 2
        const rm = (ri + ro) / 2
...
function sideGalleri () {
  ...
  h.push('<label>Status <select id="g-status"><option value="alle">Alle</option><option value="bekræftet">Bekræftet</option><option value="under undersøgelse">Under undersøgelse</option><option value="spor">Familieoplysning</option></select></label></div>')
