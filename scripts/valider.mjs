...
const STATUS = ['bekræftet', 'under undersøgelse']
...
for (const [n, p] of P) {
  ...
  if (!STATUS.includes(p.status)) fejl.push(hvem + ': status skal være "bekræftet" eller "under undersøgelse"')
  ...
  if (n > 1) {
    ...
    else if (barn.status !== 'bekræftet') fejl.push(hvem + ': barnet (anenr. ' + (n >> 1) + ') er ikke bekræftet endnu. Forældre undersøges først, når barnet er bekræftet')
  }
  ...
  for (const id of ids) { brugteKilder.add(id); if (!K.has(id)) fejl.push(hvem + ': kilden "' + id + '" findes ikke i kilder.json') }
